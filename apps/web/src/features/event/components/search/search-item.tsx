...
import { type SearchEvent } from "@features/event/types";

interface Props {
  option: AutocompleteOption<SearchEvent>;
  query?: string;
}
...
const EventSearchItem = ({ option, query }: Props) => {
  return (
    <div className="z-10 flex items-center w-full h-10 max-w-full gap-2 p-1 rounded-md cursor-pointer hover:bg-accent">
      <Image
        src={getFilePath(option.value?.bannerSmallId)}
        alt="Plakat"
        title="Plakat"
      ...
      <div className="grid w-full leading-5">
        ...
        <Highlighter
          ...
          searchWords={query?.split(" ") ?? []}
          autoEscape={true}
          textToHighlight={makeAddressString(option.value?.fullAddress) ?? ""}
        />
      </div>
    </div>
  );
